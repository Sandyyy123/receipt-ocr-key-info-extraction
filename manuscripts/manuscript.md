...
When GPU memory is shared across multiple Portfolio projects, Unsloth provides a drop-in QLoRA fine-tune path that reduces VRAM use by approximately 60% with comparable F1, at the cost of additional setup complexity. We document the substitution point in `src/model_advanced.py` so that switching between full fine-tuning and QLoRA fine-tuning is a single import change.
...
We presented a fully-specified, code-only implementation for two SROIE Task 3 pipelines: a Tesseract+regex baseline and a LayoutLMv3 token classifier. Both pipelines share the same evaluation code, the same train/val/test split, and the same BIO label generation procedure. Empirical numbers will be filled in by the build script after the main session executes `src/model_baseline.py` and `src/model_advanced.py` on a GPU node. The implementation is intended to be re-runnable end to end and to serve as a reference template for the next four projects in the Project layout

The broader ambition of this work is to give a Portfolio MLE practitioner a defensible answer to the most common question on a Mittelstand AP engagement: should we build with regex or with a fine-tuned layout-aware transformer? The implementation makes the answer empirical rather than rhetorical. Once the test-split numbers are in, the per-field F1 deltas, the CPU-versus-GPU latency, and the training cost are all accessible from the same JSON files, and the cost-quality trade-off can be presented to a non-technical sponsor in a single dashboard view. That is the v1.0 deliverable; v1.0 will extend the same template to a German Rechnung corpus and a Donut OCR-free comparison.
